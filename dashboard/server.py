...
def save_tasks(tasks):
    ...
    def _refresh():
        try:
            subprocess.run([sys.executable or 'python', str(SCRIPTS / 'refresh_live_data.py')], timeout=30)
...
def add_skill_to_agent(agent_id, skill_name, description, trigger=''):
    ...
    try:
        subprocess.run([(sys.executable or 'python'), str(SCRIPTS / 'sync_agent_config.py')], timeout=10)
    except Exception:
        pass
    ...
    def do_wake():
        try:
            ...
            for attempt in range(1, 3):
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=130, shell=IS_WINDOWS)
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    # 构造精简消息（不含多行命令，避免消息截断）
    _msgs = {
        'taizi': f'📜 皇上旨意: {title[:50]} | 任务ID: {task_id} | 请处理',
        'zhongshu': f'📜 请为以下旨意起草方案: {title[:50]} | 任务ID: {task_id}',
        'menxia': f'📋 请审议以下方案: {title[:50]} | 任务ID: {task_id}',
        'shangshu': f'📮 请派发执行: {title[:50]} | 任务ID: {task_id} {"| 建议部门: " + target_dept if target_dept else ""}',
    }
    ...
    def _do_dispatch():
        try:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=310, shell=IS_WINDOWS)
                if result.returncode == 0:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == '/api/morning-brief/refresh':
            ...
            def do_refresh():
                try:
                    cmd = [(sys.executable or 'python'), str(SCRIPTS / 'fetch_morning_news.py')]
                    if force:
                        cmd.append('--force')
        ...
        if p == '/api/set-model':
            ...
            def apply_async():
                try:
                    subprocess.run([(sys.executable or 'python'), str(SCRIPTS / 'apply_model_changes.py')], timeout=30)
                    subprocess.run([(sys.executable or 'python'), str(SCRIPTS / 'sync_agent_config.py')], timeout=10)
                except Exception as e:
...
def main():
    ...
    def _auto_sync():
        ...
        while True:
            try:
                subprocess.run([sys.executable or 'python', str(SCRIPTS / 'sync_from_openclaw_runtime.py')],
                              timeout=15, capture_output=True)
                subprocess.run([sys.executable or 'python', str(SCRIPTS / 'sync_agent_config.py')],
                              timeout=10, capture_output=True)
                subprocess.run([sys.executable or 'python', str(SCRIPTS / 'sync_officials_stats.py')],
                              timeout=10, capture_output=True)
                subprocess.run([sys.executable or 'python', str(SCRIPTS / 'refresh_live_data.py')],
                              timeout=15, capture_output=True)
            except Exception as e:
                log.warning(f'自动同步失败: {e}')
            time.sleep(5)
    threading.Thread(target=_auto_sync, daemon=True).start()
    log.info('自动同步已启动（每 5 秒）')
